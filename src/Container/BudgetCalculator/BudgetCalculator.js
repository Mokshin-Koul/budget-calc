    ...
    if (this.props.total) {
      button = this.props.token ? (
        ...
        <button
          ...
          onClick={this.authRedirectHandler}
        >
          SignUp/SignIn
        </button>
      );
      clearAllButton = (
        <button className={classes.submit} onClick={this.props.onclearState}>
          Clear All
        </button>
      );
    }
      ...
      <div>
        ...
        <div className={classes.budget}>
          <div>
            ...
            <IncomeItems
              items={this.props.incomeValues}
              desc={this.props.incomeInfo}
              remove={this.props.removeIncome}
            />
          </div>
          <div>
            <p className={classes.expense}>EXPENSE</p>
            <IncomeItems
              items={this.props.expenseValues}
              desc={this.props.expenseInfo}
              remove={this.props.removeExpense}
            />
          </div>
        </div>
...
const mapDispatchToProps = (dispatch) => {
  return {
    onAddIncome: (value, description) =>
      dispatch(actionCreators.income(value, description)),
    onAddExpense: (value, description) =>
      dispatch(actionCreators.expense(value, description)),
    removeExpense: (value, description) =>
      dispatch(actionCreators.removeExpense(value, description)),
    removeIncome: (value, description) =>
      dispatch(actionCreators.removeIncome(value, description)),
    onSubmitData: (data, token) =>
      dispatch(actionCreators.submitBudget(data, token)),
